...
class ClientHandler:
    ...
    def __init__(self, server_instance, clienthandler, addr):
        ...
        self.sendID(self.client_id, menu.get(), menu.option(self))
    ...
    def process_requests(self):
        ...
        """
        while True:
            raw_data = self.receive()  # receives data from this client
    ...
    def sendID(self, clientid, menu, option):
        """
        TODO: send the client id to the client
        """
        self.requestProcessed += 1
        data = {'clientid': clientid, 'menu': menu, 'option': option, 'connected': self.requestProcessed}
        self.send(data)
